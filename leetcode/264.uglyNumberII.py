...
1. Naive solution - exhaust and verify

Complexity: O(Nsqrt(N))

Exploit recurrence relation. A new ugly number can only be generated by multiplying
one of the factors(2,3,5) with a smaller ugly number, except for 1.
...
The problem is how to generate them in an ordered way.

2. Dynamic programming - min heap
Generate one by one breadth-first search with a priority queue search frontier.

Time complexity: O(NlogN).
Space complexity: more than O(N).

2. Dynamic programming - tuple state of three pointers
There are only three possible ways to generate a new ugly number:
    multiply a smaller ugly number by 2
    multiply a smaller ugly number by 3
    multiply a smaller ugly number by 5

Then we can maintain a tuple of three pointers as candidates to generate next
ugly number.
